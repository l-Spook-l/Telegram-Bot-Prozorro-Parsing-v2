...
async def help_user(message: types.Message, state: FSMContext):
    await message.answer(
        ...
        "\nДля параметру Регіон можна вказати лише одне значення у одному фільтрі (це обмеження Prozorro), "
        ...
        "\nСтатус: період уточнень, кваліфікація"
...
async def cancel_handler(message: types.Message, state: FSMContext):
    ...
    if current_state is None:
        ...
        await message.reply('Додавання нового фільтру скасовано',
                            reply_markup=action_menu_markup.as_markup(resize_keyboard=True))
    await state.clear()
